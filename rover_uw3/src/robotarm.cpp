...
void RobotArm::run(double encoder[18], double force[6], double driving_torque[6]){
    for(uint i = 0; i < 6; i++){
        sensor_force(i) = force[i];
    }

    for(uint i = 0; i < num_body; i++){
        body[i + 1].qi = encoder[i];
        body[i + 1].dqi = (encoder[i] - encoder[i + 6])/h;
        body[i + 1].ddqi = ((encoder[i] - encoder[i + 6])/h - (encoder[i + 6] - encoder[i + 18])/h)/h;
    }
    ...
    for(uint i = 0; i < num_body; i++){
        driving_torque[i] = body[i + 1].njip(2);
    }
}
