...
{
    ros::init(argc, argv, "encoder_sensor_talker_node");
    ros::NodeHandle nh;

    ros::Publisher pub = nh.advertise<std_msgs::Float64MultiArray>("encoder_sensor", 1);

    ros::Rate loop_rate(10);

    std::string path = ros::package::getPath("encoder_sensor_talker");
	std::string file_name = path + "/data/encoder_data.csv";
